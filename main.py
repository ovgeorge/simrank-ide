...
def block_mat_mat(parts, A):
    ...
    start = 0
    global blocks
    blocks = [0 for _ in range(parts + 1)]
    for i in range(parts + 1):
        ...
        if (i + 1) != parts or elements_in_last_part == 0:
            ...
            start += elements_in_part
    globalDict = {'A': A, 'W': W, 'threshold': threshold}
    pool = Pool(4, init_pool(globalDict))
    result = pool.map(mat_mat, range(parts))
...
def matvec_block(x, num_iter=10, threshold=1e-4):
    ...
    for _ in range(num_iter):
        XS = block_mat_mat(1000, A)  # number of blocks we partition matrix for multiplication.
...
if __name__ == "__main__":
    ...
    output_filename = "d_simplewiki_2e-2.mat"
    ...
    # d, info = solve_system(y, my_block_matvec,
    #                        call=my_callback)  # scalable version, but with performance issues due to scipy sparse matrix realization
    t2 = time.time()
